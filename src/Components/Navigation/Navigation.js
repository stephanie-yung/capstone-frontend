...
const Navigation = ({ onRouteChange, isSignedIn }) => {
    if (isSignedIn) {
      return (
        <nav style={{display: 'flex', justifyContent: 'flex-end', backgroundColor: '#b3d1b3'}}>
          <p onClick={() => onRouteChange('signout')} className=' ma4 f4 link dim white pa1 pointer'>Sign Out</p>
        </nav>
      );
    } else {
      return (
